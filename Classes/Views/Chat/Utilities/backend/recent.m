...
//-------------------------------------------------------------------------------------------------------------------------------------------------
void CreateRecentItemLocally(NSDictionary *recent)
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    PFQuery *query = [PFQuery queryWithClassName:PF_RECENT_CLASS_NAME];
    [query whereKey:PF_RECENT_USERID equalTo:[PFUser currentUser].objectId];
    [query whereKey:PF_RECENT_GROUPID equalTo:recent[@"groupId"]];
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
     {
         if (error == nil)
         {
             if ([objects count] == 0)
             {
                 ...
                 recent[PF_RECENT_USERID] = recent[@"userId"];
                 recent[PF_RECENT_GROUPID] = recent[@"groupId"];
                 recent[PF_RECENT_MEMBERS] = recent[@"members"];
                 recent[PF_RECENT_DESCRIPTION] = recent[@"description"];
                 recent[PF_RECENT_LASTUSER] = recent[@"lastUser"];
                 recent[PF_RECENT_LASTMESSAGE] = recent[@"lastMessage"];
                 recent[PF_RECENT_COUNTER] = recent[@"counter"];
                 recent[PF_RECENT_DATE] = recent[@"date"];
                 recent[PF_RECENT_PROFILEID] = recent[@"profileId"];
                 [recent pinInBackgroundWithBlock:^(BOOL succeeded, NSError *error)
                  {
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
void CreateRecentItemsLocally(NSArray *recents)
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    PFQuery *query = [PFQuery queryWithClassName:PF_RECENT_CLASS_NAME];
    [query whereKey:PF_RECENT_USERID equalTo:[PFUser currentUser].objectId];
             ...
             [PFObject unpinAllInBackground:objects block:^(BOOL success, NSError *error)
              {
                  if (success)
                  {
                      NSMutableArray *recentsLocal = [NSMutableArray alloc];
                      for (NSDictionary *recent1 in recents)
                      {
                          PFObject *recent = [PFObject objectWithClassName:PF_RECENT_CLASS_NAME];
                          recent[PF_RECENT_USERID] = recent1[@"userId"];
                          recent[PF_RECENT_GROUPID] = recent1[@"groupId"];
                          recent[PF_RECENT_MEMBERS] = recent1[@"members"];
                          recent[PF_RECENT_DESCRIPTION] = recent1[@"description"];
                          recent[PF_RECENT_LASTUSER] = recent1[@"lastUser"];
                          recent[PF_RECENT_LASTMESSAGE] = recent1[@"lastMessage"];
                          recent[PF_RECENT_COUNTER] = recent1[@"counter"];
                          recent[PF_RECENT_DATE] = recent1[@"date"];
                          recent[PF_RECENT_PROFILEID] = recent1[@"profileId"];
                          [recentsLocal addObject:recent];
                      }
                      [PFObject pinAllInBackground:recentsLocal block:^(BOOL succeeded, NSError *error)
                       {
                           if (error != nil) NSLog(@"CreateRecentItem save error.");
                       }];
                  }
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
void DeleteRecentItemsLocally(PFUser *user1, PFUser *user2)
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    PFQuery *query = [PFQuery queryWithClassName:PF_RECENT_CLASS_NAME];
...
void DeleteRecentItemLocally(PFObject *recent)
{
    [recent deleteInBackgroundWithBlock:^(BOOL succeeded, NSError *error)
     {
